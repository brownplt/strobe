...
let bind_cont cont fn = match cont with
  ...
  | Cont k ->
      ...
      let r = new_name () in
        Fix (new_node (), 
             [false, k', [r], TArrow ([TTop], TTop), k (mk_id r)],
             fn k')      
...
let rec cps_exp  (exp : exp) (throw : id) (k : cont) : cpsexp = match exp with
  ...
  | EAssertTyp (_, t, e) -> begin match k with
      ...
      | Cont k ->
          ...
          let r = new_name () in
            Fix (new_node (),
                 [true, k', [r], TArrow ([t], TTop), k (mk_id r)],
  ...
  | ELabel (_, l, _, e) ->
      let r = new_name () in
        Fix (new_node (),
             [(false, l, [r], TArrow ([TTop], TTop), ret k (mk_id r))],
  ...
  | ENew (_, constr, args) ->
      ...
      and obj = new_name () in
        Bind (new_node (), obj, Object [],
              Fix (new_node (),
                   [(false, k', [new_name ()], TArrow ([TTop], TTop), 
                     ret k (mk_id obj))],
                   cps_exp_list args throw
                     (fun argvs ->
  ...
  | ETryCatch (p, body, exn, catch_body) ->
      let throw' = mk_name "throw-cont" in
        bind_cont k
          (fun cont ->
             Fix (new_node (),
                  [false, throw', [exn], TArrow ([TTop], TBot),
